...
std::shared_ptr<TServer> thrift_server::create(proxy_ptr &proxy, const std::string &proxy_type, int port, size_t num_threads) {
    auto clone_factory = std::make_shared<thrift_handler_factory>(proxy, proxy_type);
